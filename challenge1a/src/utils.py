"""
Utility Functions for PDF Processing - Challenge 1A
...
def is_heading_candidate(text: str, font_size: float, avg_font_size: float,
                         font_flags: int, language: str = 'english') -> bool:
    if font_size <= avg_font_size or len(text) < 3 or len(text) > 200:
        return False
    is_bold = bool(font_flags & 2**4)
    patterns = {
        'english': [r'^\d+\.\s+[A-Z]', r'^Chapter\s+\d+', r'^[A-Z][A-Z\s]+$', r'^\d+\.\d+\s+'],
        'japanese': [r'^第\d+章', r'^第\d+節', r'^\d+\.\s*']
    }.get(language, [])
    matched = any(re.search(p, text) for p in patterns)
    score = (font_size - avg_font_size) * 0.1 + (0.3 if is_bold else 0) + (0.4 if matched else 0)
    return score > 0.3
